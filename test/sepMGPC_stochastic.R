...
computeEvidence_minibatch <- function(a, q, q_cavity, n_data, n_minibatch) {
  ...
  for (i in 1 : a$nK) {
    log_normalizer_q <- a$m / 2 * log(2 * pi) - sum(log(diag(q[[ i ]]$cholEinv))) + 0.5 * sum((q[[ i ]]$cholEinv %*% q[[ i ]]$m)^2)
    log_evidence <- log_evidence + (n_minibatch - sum(a$Y == i)) * (q_cavity$log_normalizer_cavity[ i ] - log_normalizer_q) +
      (a$nK - 1) * sum(a$Y == i) * (q_cavity$log_normalizer_cavity[ i ] - log_normalizer_q)
  }
...
computeGradsLikelihood <- function(a, q, q_cavity) {
  ...
  uVOldMatrix_c <- q_cavity$uV_cavity
  
  for (i in 1 : a$nK) {
    mOldMatrix_c[ ,  i, ] <- matrix(q_cavity$m_cavity[ i, ], a$n, a$m, byrow = TRUE)
    mOldMatrix_yi[ which(a$Y == i),, ] <- aperm(array(q_cavity$m_cavity[ i, ], c(a$m, sum(a$Y == i), a$nK)), c(2, 3, 1))
    uVOldMatrix_yi[ which(a$Y == i),, ] <- aperm(array(q_cavity$uV_cavity[ which(a$Y == i), i, ], c(sum(a$Y == i), a$m, a$nK)), c(1, 3, 2))
  }
...
epMGPCInternal <- function(X, Y, m, n_minibatch, Xbar_ini = NULL, log_sigma = rep(0, length(levels(Y))),autotuning = TRUE, indpoints = FALSE,
  ...
  if (!is.null(start))  {
    ...
    f1Hat <- list(Sigma = array(0, c(nK, m, m)), Mu = matrix(0, nK, m))
  }
  ...
  while (cont < max_iters) {
    ...
    for (n in sample(1 : ceiling(nBatches))) {
      ...
      a <- list(gMGPCinfo = gMGPCinfo, f1Hat = f1Hat, nK = nK, n = length(to_sel), Y = Ybatch, m = m, d = d)
      ...
      a <- refineFactors(a, q_cavity, damping, factors_per_class, nrow(Xbatch))
      
      f1Hat$Sigma  <- a$f1Hat$Sigma
      f1Hat$Mu <- a$f1Hat$Mu
      ...
      if (cont %in% c(seq(1,9),seq(10,99,5),seq(100,999,50),seq(1000,9999,500),seq(1000,100000,5000))) {
        ...
        value_log[nrow(value_log)+1,] <- list(cont,proc.time()[1]-t0[1],1-(performance$err),-(performance$neg_meanll),-(performance$neg_medll),elbo0,performance$auc,performance$ece,performance$mce)
...
cavityDistribution_minibatch <- function(a, q, factors_per_class){
  ...
  utEu_yi_cav <- utEu_c_cav <- utm_c_cav <- utm_yi_cav <- matrix(0, a$n, a$nK)
  sigma <- sigma0 <- rep(0, a$nK)
  KnmKmmInvKmn <- matrix(0, a$n, a$nK)
  log_normalizer_cavity = rep(0, a$nK)
  m_cavity <- matrix(0, a$nK, a$m)
  uV_cavity <- array(0, c(a$n, a$nK, a$m))
  
  for (i in 1 : a$nK) {
    
    M <- a$gMGPCinfo[[ i ]]$KmmInv + a$f1Hat$Sigma[ i, , ] * (factors_per_class[ i ] - 1) / factors_per_class[ i ]
    cholEinv <- chol(M)
    E <- chol2inv(cholEinv)
    s <- a$f1Hat$Mu[ i, ] * (factors_per_class[ i ]-  1) / factors_per_class[ i ]
    m <- E %*% s
    
    uE <- a$gMGPCinfo[[ i ]]$KnmKmmInv %*% E
    utEu_c_cav[ , i ] <- rowSums(uE * a$gMGPCinfo[[ i ]]$KnmKmmInv)
    utm_c_cav[ ,i ] <- a$gMGPCinfo[[ i ]]$KnmKmmInv %*% m
    utEu_yi_cav[ which(a$Y == i), ] <- matrix(utEu_c_cav[ which(a$Y == i), i ], sum(a$Y == i), a$nK)
    utm_yi_cav[ which(a$Y == i), ] <- matrix(utm_c_cav[ which(a$Y == i), i ], sum(a$Y == i), a$nK)
    sigma[ i ] <- a$gMGPCinfo[[ i ]]$sigma
    sigma0[ i ] <- a$gMGPCinfo[[ i ]]$sigma0
    KnmKmmInvKmn[ ,i ] <- a$gMGPCinfo[[ i ]]$KnmKmmInvKmn
    
    log_normalizer_cavity[ i ] <- a$m / 2 * log(2 * pi) - sum(log(diag(cholEinv))) + 0.5 * sum(m * (M %*% m))
    
    m_cavity[ i, ] <- m
    uV_cavity[ ,i, ] <- uE
  }
  
  q_cavity <- list(utEu_c = utEu_c_cav, utm_c = utm_c_cav, utEu_yi = utEu_yi_cav, utm_yi = utm_yi_cav,
                   log_normalizer_cavity = log_normalizer_cavity, m_cavity = m_cavity, uV_cavity = uV_cavity)
...
evaluate_test_performance <- function(ret, X_test, Y_test, q = NULL) {
  ...
  list(err = err, neg_meanll = neg_meanll, neg_medll = neg_medll, auc=auc, ece=ece, mce=mce)
...
reconstructPosterior <- function(a){
  ...
  for (i in 1 : a$nK) {
    ...
    s <- a$f1Hat$Mu[ i, ]
    qTemp$m <- qTemp$E %*% s
    qTemp$utEu <- rowSums((a$gMGPCinfo[[ i ]]$KnmKmmInv %*% qTemp$E) * a$gMGPCinfo[[ i ]]$KnmKmmInv)
    qTemp$utm <- a$gMGPCinfo[[ i ]]$KnmKmmInv %*% qTemp$m
    q[[ i ]] <- qTemp
...
#
#
refineFactors <- function(a, q_cavity, damping, factors_per_class, n_minibatch){
  
  C2_yi <- C2_c <- C1_yi <- C1_c <- matrix(0, a$n, a$nK)
  ...
  C1_c[ cbind(1 : a$n, a$Y) ] <- 0
  C2_c[ cbind(1 : a$n, a$Y) ] <- 0
  C1_yi[ cbind(1 : a$n, a$Y) ] <- 0
  C2_yi[ cbind(1 : a$n, a$Y) ] <- 0
  
  for (i in 1 : a$nK) {
    
    u_c <- a$gMGPCinfo[[ i ]]$KnmKmmInv
    Sigma_new <- t(u_c * matrix(C1_c[ , i ], a$n, a$m)) %*% u_c + t(u_c[ sort(rep(which(a$Y == i), a$nK - 1)), ] *
                                                                      matrix(c(t(C1_yi[ which(a$Y == i), - i ])), sum(a$Y == i) * (a$nK - 1), a$m)) %*% u_c[ sort(rep(which(a$Y == i), a$nK - 1)), ]
    Mu_new <- C2_c[ , i ]  %*% u_c + rowSums(C2_yi[ which(a$Y == i), - i, drop = FALSE ]) %*% u_c[ which(a$Y == i) ,  ]
    
    n_factors_minibatch_i <- n_minibatch - sum(a$Y == i) + sum(a$Y == i) * (a$nK - 1)
    
    a$f1Hat$Sigma[ i, , ]  <- (damping * Sigma_new + (1 - damping) * a$f1Hat$Sigma[ i, , ] * n_factors_minibatch_i / factors_per_class[ i ]) +
      (factors_per_class[ i ] - n_factors_minibatch_i) / factors_per_class[ i ] * a$f1Hat$Sigma[ i, , ]
    a$f1Hat$Mu[ i, ]  <- (damping * Mu_new + (1 - damping) * a$f1Hat$Mu[ i , ] * n_factors_minibatch_i / factors_per_class[ i ]) +
      (factors_per_class[ i ] - n_factors_minibatch_i) / factors_per_class[ i ] * a$f1Hat$Mu[ i, ]
